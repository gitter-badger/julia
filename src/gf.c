...
*/
static jl_methlist_t *jl_method_cache_assoc_by_type_(jl_methlist_t *ml, jl_tupletype_t *types, int8_t inexact, jl_svec_t **env)
{
    ...
    while (ml != (void*)jl_nothing) {
        ...
        if (lensig == n || (ml->va && lensig <= n+1)) {
            ...
            if (ml->isleafsig)
                ...
                jl_value_t *ti = lookup_match((jl_value_t*)types, (jl_value_t*)ml->sig, env, ml->tvars);
                ismatch = (ti != (jl_value_t*)jl_bottom_type);
                if (ismatch) {
                    // parametric methods only match if all typevars are matched by
                    // non-typevars.
                    size_t i, l;
                    for (i = 0, l = jl_svec_len(*env); i < l; i++) {
